...
private:
    ...
    void setLOB(PNG & im, pair<int,int> loc, int dd);
    ...
    // Given the decimal value of the color channel (colChannel), and two bits (dVal), 
    // replaces the last two bits of the binary representation of colChannel
    // and returns the new value of the color channel in decimal representation 
    int newVal(int colChannel, int dVal);
